    ...
    reducers: {
                ...
                state.ghosts.forEach((ghost) => {
                    if (ghost.shallTick) {
                        if (ghost.isSmart && !ghost.isEdible) {
                            ghost.movementDirection = getMovementDirectionByPosition(state.hackman.position, ghost.position);
                            ghost.direction = getDirectionByMovementDirection(ghost.movementDirection, gameFieldForAll, ghost, ghosts);
                            gameFieldForAll = moveGhostSmart(gameFieldForAll, ghost, ghosts, state.hackman);
                        } else if (ghost.isSmart && ghost.isEdible) {
                            ghost.movementDirection = getMovementDirectionByPositionRevert(state.hackman.position, ghost.position);
                            ghost.direction = getDirectionByMovementDirection(ghost.movementDirection, gameFieldForAll, ghost, ghosts);
                            gameFieldForAll = moveGhostSmart(gameFieldForAll, ghost, ghosts, state.hackman);
                        } else {
                            // dumb move
                            ...
                            gameFieldForAll = moveGhostDumb(gameFieldForAll, ghost, ghosts, state.hackman);
                        }
                    }
                });
        ...
        changeIsMoveableHackman: (state, payload: PayloadAction<Direction>) => {
            ...
            if (isMoveable === Moveable.Yes) {
                if (state.hackman.hackmanMoved === false) state.hackman.hackmanMoved = true;
            }
            ...
            switch (direction) {
                case Direction.Up:
                    if (state.hackman.canSetBlock && state.gameField[state.hackman.position.y + 1][state.hackman.position.x] === Empty) {
                        state.gameField = generateBlockOnGameField(state.hackman.position.y + 1, state.hackman.position.x, state.gameField);
                ...
                case Direction.Left:
                    if (state.hackman.canSetBlock && state.gameField[state.hackman.position.y][state.hackman.position.x + 1] === Empty) {
                        state.gameField = generateBlockOnGameField(state.hackman.position.y, state.hackman.position.x + 1, state.gameField);
                        state.hackman.canSetBlock = false;
                ...
                if (state.hackman.moveable !== Moveable.No && state.hackman.moveable !== Moveable.Gate) {
                    ...
                    if (positionToCheck.x !== 100) {
                        if (state.gameField[positionToCheck.y][positionToCheck.x] === Coin || state.gameField[positionToCheck.y][positionToCheck.x] === Empty) {
                            ...
                            state.hackman.moveable = canMove(state.gameField, state.hackman.position, state.hackman.direction, state.ghosts);
        ...
        deleteBlock: (state) => {
            state.gameField = setGameField(state.block[0], state.block[1], state.gameField);
